...
$values = &$this->values;
		...
		$sep = 0;
		if ($this->params->get('display_name', 1)) {
			if ($values->username != '') {
				echo phocaguestbookHelperFront::wordDelete($values->username, 40, '...');
				$sep = 1;
			}
		}
		if ($this->params->get('display_email', 1)) {
			if ($values->email != '') {
				if ($sep == 1) {
		...
		if ($this->params->get('display_website', 1)) {
			if ($values->homesite != '') {
		...
		<small class="pgb_thi_font"><?php echo JHTML::_('date',  $values->date, JText::_( $this->params->get('date_format', 'DATE_FORMAT_LC') ) ); ?>
